	...
	class CT_DB_Registration {
		...
		public function login_user() {
			...
			if( isset( $_POST['ctdb_user_login'] ) && isset( $_POST['ctdb_login_nonce'] ) && wp_verify_nonce( $_POST['ctdb_login_nonce'], 'ctdb-login-nonce' ) ) {
				...
				if( empty( $errors ) ) {
					...
					if( $redirect_to && ! current_user_can( 'manage_plugins' ) ) {
						$url = esc_url( add_query_arg( 'redirecting', 'true', get_permalink( $redirect_to ) ) );
					} else {
						$url = esc_url( add_query_arg( 'redirecting', 'true', get_permalink() ) );
					}
					wp_redirect( $url );
					exit;
		...
		public function redirect_to_login_page() {
			...
			if( 'wp-login.php' == $pagenow && isset( $options['hide_wp_login'] ) && isset( $options['frontend_login_page'] ) ) {
				$redirect_url = esc_url( add_query_arg( 'login', 'bounced', get_permalink( $options['frontend_login_page'] ) ) );
